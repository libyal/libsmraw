...
then
	docker run -t -v "${PWD}:/libsmraw" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libsmraw && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libsmraw;
