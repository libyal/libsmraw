...
#if defined( _MSC_VER )
#define smrawtools_path_make( path, path_size, drive, directory, filename, extension ) \
	_wmakepath_s( path, path_size, drive, directory, filename, extension )

#define smrawtools_path_split( path, drive, drive_size, directory, directory_size, filename, filename_size, extension, extension_size ) \
...
{
	static char *function = "smrawtools_glob_initialize";
...
int smrawtools_glob_free(
     ...
     libcerror_error_t **error )
{
	static char *function = "smrawtools_glob_free";
     ...
     libcerror_error_t **error )
{
	...
	static char *function = "smrawtools_glob_resize";
...
{
#if defined( HAVE_IO_H ) || defined( WINAPI )
	smrawtools_find_data_t find_data;
...
#endif
	static char *function   = "smrawtools_glob_resolve";
	     ...
	     iterator++ )
	{
		...
		smrawtools_path_split(
		 ...
		 _MAX_EXT );
#endif
		find_handle = smrawtools_find_first(
		               patterns[ iterator ],
		               &find_data );
