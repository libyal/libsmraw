/*
 * smioverify
 * Verifies the integrity of the media data within storage media file(s)
 *
...
verification_handle_t *smioverify_verification_handle = NULL;
int smioverify_abort                                  = 0;
      ...
      FILE *stream )
{
	...
	fprintf( stream, "Use smioverify to verify data stored in storage media file(s).\n\n" );

	fprintf( stream, "Usage: smioverify [ -d digest_type ] [ -l log_filename ]\n"
	                 "                  [ -p process_buffer_size ] [ -hqvVw ] files\n\n" );
	...
	fprintf( stream, "\t-w:    wipe process buffer on read error\n" );
}
 ...
 */
ssize64_t smioverify_read_input(
           ...
           liberror_error_t **error )
{
	...
	static char *function                  = "smioverify_read_input";
...
/* Signal handler for smioverify
 */
void smioverify_signal_handler(
      libsystem_signal_t signal )
{
	liberror_error_t *error = NULL;
	static char *function   = "smioverify_signal_handler";

	smioverify_abort = 1;

	if( ( smioverify_verification_handle != NULL )
	 && ( verification_handle_signal_abort(
	       smioverify_verification_handle,
	       &error ) != 1 ) )
	{
...
#endif
{
	...
	libsystem_character_t *program                     = _LIBSYSTEM_CHARACTER_T_STRING( "smioverify" );
	...
	while( ( option = libsystem_getopt(
	                   argc,
	                   argv,
	                   _LIBSYSTEM_CHARACTER_T_STRING( "d:hl:p:qvVw" ) ) ) != (libsystem_integer_t) -1 )
	{
		switch( option )
		{
	...
	if( smioverify_abort == 0 )
	{
		 ...
		 */
		verify_count = smioverify_read_input(
		                verification_handle,
		                wipe_block_on_error,
		                (size_t) process_buffer_size,
		                process_status,
	...
	if( status == PROCESS_STATUS_COMPLETED )
	{
		...
		if( verification_handle_get_amount_of_read_errors(
		     verification_handle,
		     ...
		     &error ) != 1 )
		{
			fprintf(
			 stderr,
			 "Unable to determine the amount of read errors.\n" );
		...
		if( verification_handle_read_errors_fprint(
		     verification_handle,
		     stdout,
		     &error ) != 1 )
		{
		...
		if( ( log_file_stream != NULL )
		 && ( verification_handle_read_errors_fprint(
		       verification_handle,
		       log_file_stream,
		       &error ) != 1 ) )
		{
		...
		if( verification_handle_additional_integrity_hash_values_fprint(
		     verification_handle,
		     stdout,
		     &error ) != 1 )
		{
		...
		if( ( log_file_stream != NULL )
		 && ( verification_handle_additional_integrity_hash_values_fprint(
		       verification_handle,
		       log_file_stream,
		       &error ) != 1 ) )
		{
